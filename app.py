...
def forgot_reset():
 ...
 update_where('admin_accounts',{'username':f'eq.{ADMIN_USERNAME}'},{'password_hash':generate_password_hash(pw),'updated_at':datetime.now(timezone.utc).isoformat()})
 delete_where('admin_password_reset',{'id':f"eq.{x['id']}"})
...
def change_password():
 ...
 update_where('admin_accounts',{'username':f'eq.{ADMIN_USERNAME}'},{'password_hash':generate_password_hash(new),'updated_at':datetime.now(timezone.utc).isoformat()})
...
@require_admin
def admin_update():
 d=request.get_json(silent=True) or {};i=d.get('id');keys=['fixture_no','item_no','description','bom_qty','no_of_sets','total_qty','received','status'];p={k:d[k] for k in keys if k in d}
...
@app.get('/api/export')
def export():
 path=os.path.join(os.path.dirname(__file__),'CORVA_TRACKER_TEMPLATE.xlsx')
 wb=load_workbook(path);rows=get('fixtures',{'select':'*','order':'id.asc'});by={(str(x['sheet']),str(x['fixture_no']).strip(),str(x['item_no']).strip()):x for x in rows};groups={}
